...
import { Award, Printer, School, CheckCircle } from 'lucide-react';
  ...
  if (!data) {
    return (
      <EmptyState
        ...
        description="Grades have not yet been published for your account."
        icon={Award}
      />
    );
  }

  const { student, overallPercentage, overallGrade, totalMarks, totalMaxMarks, subjects } = data;
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between no-print">
        <PageHeader
          ...
          subtitle="Spring 2026 Semester Grade Report & Performance Record"
        />
        <button
          onClick={handlePrint}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-8 sm:p-12 text-slate-800 dark:text-slate-100 print:shadow-none print:border-0 print:p-0">
        {/* School Header */}
        ...
        <div className="my-8 p-6 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Total Score: {totalMarks} out of {totalMaxMarks} points
            </p>
          </div>
