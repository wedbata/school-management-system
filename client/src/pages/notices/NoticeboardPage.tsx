...
export const NoticeboardPage: React.FC = () => {
  ...
  const handleOpenViewModal = (notice: any) => {
  ...
  const handleUpdateNotice = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await api.put(`/notices/${selectedNotice.id}`, editForm);
      if (res.data.success) {
  ...
  const handleDeleteNotice = async (id: string, title: string) => {
    if (!window.confirm(`Are you sure you want to delete circular "${title}"?`)) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Official school-wide communications, academic notices, event alerts, and institutional circulars"
        action={
          canManage && (
            <button
            ...
            >
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-400">
                ...
                <div className="flex items-center gap-1">
                  {/* View / Print Circular Modal */}
                  <button
                    onClick={() => handleOpenViewModal(notice)}
                    className="p-1 rounded-lg text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition cursor-pointer"
                    title="View & Print Circular"
                  >
                  ...
                  {canManage && (
                    <button
                      ...
                      className="p-1 rounded-lg text-slate-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/50 transition cursor-pointer"
                      title="Edit Notice"
                    >
      ...
      {/* View Notice & Print Official Memo Modal */}
      <Modal
        isOpen={isViewModalOpen}
        onClose={() => setIsViewModalOpen(false)}
        title="Official School Circular & Bulletin"
      >
        {selectedNotice && (
          <div className="space-y-6">
            {/* Memo Document */}
            <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
              {/* Header */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">
                      Office of the Principal & Academic Administration
                    </p>
                  </div>
                </div>
                <Badge variant={priorityBadgeVariant(selectedNotice.priority)} size="sm">
                  {selectedNotice.priority} PRIORITY
                </Badge>
              </div>

              {/* Memo Meta */}
              <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 dark:bg-slate-800/50 p-3.5 rounded-xl">
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-semibold">
                    Issued By
                  </span>
                  <p className="font-bold text-slate-900 dark:text-white">
                    {selectedNotice.author?.firstName} {selectedNotice.author?.lastName}
                  </p>
                  <p className="text-slate-500 text-[11px]">Role: {selectedNotice.author?.role}</p>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-semibold">
                  ...
                  <p className="font-bold text-indigo-600 dark:text-indigo-400">
                    Audience: {selectedNotice.targetRole}
                  </p>
                  <p className="text-slate-500 text-[11px]">
                    Date: {new Date(selectedNotice.createdAt).toLocaleDateString('en-US', {
                      month: 'long',
                      day: 'numeric',
                      year: 'numeric',
                    })}
              ...
              <div className="space-y-3">
                <h4 className="text-base font-bold text-slate-900 dark:text-white border-l-4 border-indigo-600 pl-3">
                  {selectedNotice.title}
                </h4>
                <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-line p-2">
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
                ...
                <span>Authorized Digital Publication</span>
              </div>
            </div>
